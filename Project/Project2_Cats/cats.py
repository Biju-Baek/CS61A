...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    typed_len = len(typed)
    num_characters = typed_len / 5
    elapsed_minutes = elapsed / 60
    return num_characters / elapsed_minutes
    # END PROBLEM 4
...
def feline_fixes(typed, source, limit):
    ...
    1
    >>> feline_fixes("rangec", "rungs", big_limit)  # Substitute: a -> u, e -> s
    2
...
def report_progress(typed, source, user_id, upload):
    ...
    upload({'id': id, 'progress': progress})
    return progress
    # END PROBLEM 8
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    times = [[x[i+1]-x[i] for i in range(len(x)-1)] for x in timestamps_per_player]
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        ans_list[times.index(min(times))].append(get_word(match, i))
