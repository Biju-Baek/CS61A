...
def sus_points(score):
    ...
    # BEGIN PROBLEM 4
    i = score
    if num_factors(i) == 3 or num_factors(i) == 4:
...
def sus_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    factors_count = num_factors(new_score)
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    while num_rolls <= 10:
        ...
        curr_avg_score = averaged_roll_dice(num_rolls, dice)
        # check if this is the best score so far or if it's tied with the current best
        if curr_avg_score >= max_avg_score:
            max_avg_score = curr_avg_score
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    num_rolls = 0
    while num_rolls <= 2:
        potential_score = score - sus_update(num_rolls, score, opponent_score)
        if potential_score >= Goal_score:
            return num_rolls
        num_rolls += 1
    return 2
    # 2 is a placeholder fo a default strategy
    # END PROBLEM 12
